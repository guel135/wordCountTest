...
//		readFile(fileName);
	...
	private static void readFolders(String folder) throws IOException {
		try(Stream<Path> paths = Files.walk(Paths.get(folder))) {
		    paths.forEach(filePath -> {
		    	if(Files.isDirectory(filePath)) System.out.println("Directory "+filePath);
	...
	private static GeneralFile readFile(String fileName) throws IOException {
		...
		Map<String, Long> result=new HashMap<>();
		Files.lines(Paths.get(fileName), Charset.forName(CHARSET_ISO)).filter(line -> 
				line.length() > 0)
				.forEach(String-> {
...
//		if (sumWords>1000) {
//			BigFile file;
//			result.entrySet().stream().filter(Long::Long>50)
//		}
